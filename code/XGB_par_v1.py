...
def predict_proba_kernel1(X, features_arr, values_arr, n_classes, block_size, n_streams=32):
    cuda_streams = [cuda.stream() for _ in range(n_streams)]

    d_result = cuda.device_array((n_classes, X.shape[0]))

    d_Xt = cuda.to_device(X.T, cuda_streams[0])
    d_features = cuda.to_device(features_arr, cuda_streams[1])
    d_values = cuda.to_device(values_arr, cuda_streams[2])
